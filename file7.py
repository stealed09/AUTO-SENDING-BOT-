...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
            ...
            try:
                await context.bot.send_message(
                    ...
                    text=(
                        ...
                        f"📛 Username: {html.escape(pending_users[user_id]['username'])}\n\n"
    ...
    if is_admin(user_id):
        ...
        kb = menu.inline_keyboard + [[InlineKeyboardButton("🛡 Admin Panel", callback_data="admin_panel")]]
...
async def broadcast_for_user(user_id: int, scheduled_time: str) -> None:
    ...
    try:
        await bot.send_message(
            chat_id=user_id,
            text=f"⏰ <b>Scheduled Broadcast Report</b> ({scheduled_time} IST)\n\n{report}",
            parse_mode="HTML",
        )
    except Exception as e:
...
async def handle_admin_callbacks(query, user_id: int, data: str, context) -> bool:
    ...
    if data == "admin_panel":
        ...
        for uid, info in pending_users.items():
            label = f"👤 {info['name']} ({info['username']}) — ID:{uid}"
            keyboard.append([
        ...
        for uid in approved_users:
            ...
            keyboard.append([InlineKeyboardButton(f"🚫 Revoke {name}", callback_data=f"revoke_{uid}")])
        keyboard.append([InlineKeyboardButton("🔙 Back", callback_data="admin_panel")])
        await query.edit_message_text(
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_approved(user_id):
        await update.message.reply_text(
            "⛔ You don't have access. Send /start to request.",
        )
        return
    ...
    if state == "awaiting_channels":
        ...
        parts = []
        if valid:
            parts.append(f"✅ Times (IST): <code>{', '.join(valid)}</code>")
        if errors:
